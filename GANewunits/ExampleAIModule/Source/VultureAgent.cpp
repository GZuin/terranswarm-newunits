...
VultureAgent::VultureAgent(Unit u) : gameUnit(u), state(NO_TASK), latencyFrames(10){
...
/*
void VultureAgent::setParameters(map<int, double>& params){
	parameters = params;
}
*/

void VultureAgent::onFrame(unordered_map<TaskType, vector<Task>*> taskMap, unordered_map<int, VultureAgent*> colleagues){
	...
	for(auto taskIter = taskMap.begin(); taskIter != taskMap.end(); ++taskIter){
		...
		/*case Explore:
			capability = float(parameters[K_VULTURE_EXPLORE]);
...
void VultureAgent::attack(Position theTarget, unordered_map<int, VultureAgent*> colleagues){
	state = PACKING; //prepares to join colleagues and then attacks
	target = theTarget;
	attack(colleagues);
}

void VultureAgent::attack(unordered_map<int, VultureAgent*> colleagues){
	...
	if(packSize >= parameters[M_PACK_SIZE] || colleaguesAround == packSize || enemiesInSight.size() > 0) {
		...
		//tries to get close to the vulture with the lowest ID around
...
Unit VultureAgent::oldestColleagueAround(){
	...
	for(auto colleague = colleagues.begin(); colleague != colleagues.end(); colleague++){
		if(colleague->getType() == UnitTypes::Terran_Vulture && colleague->getID() < minID){
...
/*
void VultureAgent::onTask(unordered_map<TaskType, list<Task>*> taskMap){
	...
	for(unordered_map<TaskType, list<Task>*>::iterator iter = taskMap.begin(); iter != taskMap.end(); ++iter){
		...
		// Task Vulture can do = Explore, Attack, GuardBase
		// Task from Task.h
		if(taskType == Explore){
...
bool VultureAgent::isOnAttack(){
	return state == PACKING || state == ATTACKING;
}

bool VultureAgent::goScout(){
...
Position VultureAgent::getPositionToScout(){
	...
	while (length < Broodwar->mapWidth()) {
		...
		if (x >= 1 && x < Broodwar->mapWidth()-1 
			&& y >= 1 && y < Broodwar->mapHeight()-1 
