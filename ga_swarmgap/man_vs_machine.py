...
def go(enemy):
    ...
    paths.inicopy('bwapi_man-%s-vs-machine.ini' % enemy)
    ...
    results_path = os.path.join(sc_dir, 'results_vs-man.txt')
    ...
    open(results_path, 'a').close() #create empty results.txt file if not exists
    ...
    print 'Matches finished. Check the result in %s.' % results_path
